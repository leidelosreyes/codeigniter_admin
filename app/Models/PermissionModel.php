...
class PermissionModel extends Model
{
    ...
    protected $allowedFields = ['controller_method', 'perm_desc'];
    ...
    public function get_datatable_data(int $limit = 0, int $offset = 0, string $orderby = null, string $orderdir = 'ASC', string $search = null )
    {
        ...
        $dataset->select('id, controller_method, perm_desc, created_at, updated_at');
        
        if($search != null)
        {
            $dataset->groupStart();
            $dataset->like('id', $search);
            $dataset->orLike('controller_method', $search);
            $dataset->orLike('perm_desc', $search);
            $dataset->groupEnd();
        }
