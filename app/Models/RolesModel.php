...
class RolesModel extends Model
{
    ...
    protected $allowedFields = ['role_name', 'role_desc', 'permission_id'];
    ...
    public function get_datatable_data(int $limit = 0, int $offset = 0, string $orderby = null, string $orderdir = 'ASC', string $search = null )
    {
        ...
        $dataset->select('id, role_name, role_desc, permission_id, created_at, updated_at');
        
        if($search != null)
        {
            $dataset->groupStart();
            $dataset->like('id', $search);
            $dataset->orLike('role_name', $search);
            $dataset->orLike('role_desc', $search);
            $dataset->orLike('permission_id', $search);
            $dataset->groupEnd();
        }
