 ...
 */
class Gbhvip399Model extends Model
{
    protected $table      = 'gbhvip399';
    ...
    protected $allowedFields = ['item', 'value', 'title'];
    ...
    public function get_datatable_data(int $limit = 0, int $offset = 0, string $orderby = null, string $orderdir = 'ASC', string $search = null )
    {
        ...
        $dataset->select('id, item, value, title, created_at, updated_at');
        
        if($search != null)
        {
            $dataset->groupStart();
            $dataset->like('id', $search);
            $dataset->orLike('item', $search);
            $dataset->orLike('value', $search);
            $dataset->orLike('title', $search);
    ...
    /**
     * Basic Model Function. Get data by setting parameters by like.
     * 
     * @param array $data Array key value pairs you wish to use to filter data.
     * @param string $side string where to apply like condition: both, before, after
     * @param int $limit Number of records to return
     * @param int $offset Offset of records to return
     * 
     * @return array
     */
    public function get_data_params_like(array $data = [], string $side = 'both', int $limit = 0, int $offset = 0)
    {
        $dataset = $this;
        foreach( $data as $key => $value )
        {
            $dataset->like($key, $value, $side);
        }
        return $dataset->findAll($limit, $offset);
        //return [$side];
    }
