...
class AdsModel extends Model
{
    ...
    protected $allowedFields = ['link', 'description', 'images'];
    ...
    public function get_datatable_data(int $limit = 0, int $offset = 0, string $orderby = null, string $orderdir = 'ASC', string $search = null )
    {
        ...
        $dataset->select('id, link, description, images');

        if($search != null)
        {
            $dataset->groupStart();
            $dataset->like('id', $search);
            $dataset->orLike('link', $search);
            $dataset->orLike('description', $search);
            $dataset->orLike('images', $search);
