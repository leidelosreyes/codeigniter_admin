...
class SysConfigModel extends Model
{
    ...
    protected $allowedFields = ['name', 'sys_desc', 'sys_value'];
    ...
    public function get_datatable_data(int $limit = 0, int $offset = 0, string $orderby = null, string $orderdir = 'ASC', string $search = null )
    {
        ...
        $dataset->select('id, name, sys_desc, sys_value, created_at, updated_at');
        
        if($search != null)
        {
            $dataset->groupStart();
            $dataset->like('id', $search);
            $dataset->orLike('name', $search);
            $dataset->orLike('sys_desc', $search);
            $dataset->orLike('sys_value', $search);
