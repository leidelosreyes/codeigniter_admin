...
class MessageModel extends Model
{
    ...
    public function get_datatable_data(int $limit = 0, int $offset = 0, string $orderby = null, string $orderdir = 'ASC', string $search = null )
    {
        ...
        $dataset->select('id,mobile_number,created_at');

        if($search != null)
        {
            $dataset->groupStart();
            $dataset->like('id', $search);
            $dataset->orLike('mobile_number', $search);
            // $dataset->orLike('deposit', $search);
            // $dataset->orLike('status', $search);
            //$dataset->orLike('notes', $search);
    ...
    {
        $dataset = $this;
        $dataset->select('mobile_number');
        return $dataset->findAll();
    }

    // public function get_distinct_sum()
    // {
    //     $dataset = $this;
    //     $dataset->select('username, sum(deposit) as deposit');
    //     $dataset->groupBy('username');
